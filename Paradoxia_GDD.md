...
- Stale `.inc` files with spaces in names (`events 2.inc`, `layouts 2.inc`, etc.) renamed to underscored equivalents to prevent make splitting them into phantom dependencies
- Makefile comma bug fixed — `-include $(ALL_TUTORS_JSON), $(ALL_TEACHING_TYPES_JSON),` had trailing commas causing `data/maps/events` phantom target; corrected to space-separated
- JSON include of `all_tutors.json` commented out from Makefile — was being `-include`d as a makefile, causing parse errors
- `asm/macros/map.inc` `.ifdef` macro replaced with direct byte output — the `.ifdef \map_id` pattern is incompatible with C preprocessor `#define` stubs (preprocessor eats the symbol name before the assembler sees it); macro now outputs bytes directly without checking
- `data/scripts/contest_hall.inc` and `data/scripts/berry_blender.inc` removed from `data/event_scripts.s` — `switch VAR_RESULT` uses a non-constant `.if` expression incompatible with the assembler
- All deleted-map `LOCALID_*` constants stubbed in `include/global.h` — covers CONTESTANT_1–4, FARAWAY_ISLAND_MEW, BIRTH_ISLAND_EXTERIOR_ROCK, all MART_CLERKs, TRUCK_BOX_TOP/BOTTOM_L/BOTTOM_R, ROUTE111/118/120 GABBY/TY/PLAYER_FALLING, UNION_ROOM_PLAYER_1–8, LAYOUT_SS_TIDAL_* constants
- poryscript 3.6.0 installed at `tools/poryscript` (Mac ARM64 binary); `command_config.json` and `font_config.json` copied from release zip to project root
- All `.pory` files compiled manually to `.inc` — `SSEnnaDeck/scripts.pory` had inverted backtick syntax corrected; closing backtick newline added
- `data/event_scripts.s` pruned — all deleted-map `.include` lines removed using Python script; only Route109, SSEnnaDeck, BramblewoodTown, Route1000, and LittlerootTown_* maps retained
- `data/layouts/layouts_2.inc` and `layouts_table_2.inc` renamed from space-named versions
- Build now reaches linker stage — all compilation errors resolved
- ~350 linker undefined references identified (full list captured) — all from deleted Battle Frontier, vanilla script maps, and deleted layout/LOCALID references; stubbing strategy confirmed for next session

## Session 15 Changes
- Route109 dock scene trigger confirmed working — VAR_ENNA_INTRO_STATE == 22 map script fires correctly on arrival
- Lab scene trigger fixed — root cause was stale .inc files out of sync with .pory sources; all four map scripts recompiled with poryscript
- VAR_ENNA_INTRO_STATE clobber bug traced and fixed — setvar in EventScript_ResetAllMapFlags was firing after the deck scene due to RunScriptImmediately queuing behaviour; fix moved VarSet(VAR_ENNA_INTRO_STATE, 21) to src/new_game.c directly before WarpToTruck(), and removed setvar from new_game.inc
- Transition and LittlerootTown exterior steps removed from VAR_ENNA_INTRO_STATE chain — chain simplified; LittlerootTown OnFrame table removed entirely from LittlerootTown/scripts.pory
- Stale .inc duplicate files (named "events 2.inc", "header 2.inc", "connections 2.inc") purged from data/maps — generated by Porymap save collision; removed with `find data/maps -name "* 2.inc" -delete`
- LOCALID_ROUTE109_BRINEY (value 1) and LOCALID_ROUTE109_BOAT (value 2) confirmed in paradoxia_localids.h — injected defines removed from Route109/scripts.inc and DewfordTown/scripts.inc after confirming raw .inc files cannot see paradoxia_localids.h via preprocessor; DewfordTown/scripts.inc now has defines injected at top directly
- LittlerootTown_EventScript_ArrivalScene stub added to LittlerootTown/scripts.inc to satisfy linker — frame table referencing it subsequently removed, stub retained for safety
- Bramblewood exterior crash at x6 identified — MB_SECRET_BASE metatile behaviour on house tile; fix: replace with correct wall metatile in Porymap
- Blue water tiles on Bramblewood exterior (top-right house area) flagged for metatile behaviour audit — potential MB_WATER crash source
- PorySuite-Z noted (github.com/InnerMobius/PorySuite-Z) — FireRed only, not compatible with pokeemerald Expansion; bookmarked for future reference

## Session 14 Changes
- Resolved all linker errors from Session 13 that were blocking the build: missing flags (FLAG_HIDE_DOCK_DOC_B/A/BEA added to flags.h at 0x3E8–0x3EA), missing LOCALIDs (LOCALID_LITTLEROOT_MOM/TWIN/BIRCH/RIVAL, LOCALID_ROUTE109_BRINEY/BOAT, LOCALID_BIRCHS_LAB_BIRCH/RIVAL/CYNDAQUIL_BALL/TOTODILE_BALL/CHIKORITA_BALL), missing VAR_ENNA_INTRO_STATE (added at 0x40F7 replacing VAR_UNUSED_0x40F7), missing SSEnnaDeck_MapScripts and Transition_MapScripts from data/event_scripts.s
- Vanilla NPC stubs added as persistent hidden objects to LittlerootTown/map.json, Route109/map.json, and LittlerootTown_ProfessorBirchsLab/map.json — all hidden via existing vanilla flags, zero runtime cost
- Special_ParadoxiaDoNamingScreen registered in data/specials.inc and declared in include/paradoxia.h
- Task_ParadoxiaGenderSelect_Init declared in include/paradoxia.h and wired into main_menu.c — replaces Task_NewGameBirchSpeech_Init at the new game branch point
- WarpToTruck() call in NewGameInitData() moved to after RunScriptImmediately(EventScript_ResetAllMapFlags) — fixes ordering issue where VAR_ENNA_INTRO_STATE was not set before the warp fired
- New game spawn changed from MAP_INSIDE_OF_TRUCK to MAP_SSENNA_DECK at coordinates 4,10 (player faces right)
- VAR_ENNA_INTRO_STATE confirmed at memory address 0x020371EE (0x40F7 offset from vars base)
- SSEnnaDeck/scripts.pory raw block removed — map_script/map_script_2 directives are native poryscript, not raw assembly
- Route109/scripts.pory wrapped in single raw block — entire file is raw assembly; poryscript compiler requires -cc and -fc flags: `tools/poryscript/poryscript -i [input] -o [output] -cc tools/poryscript/command_config.json -fc tools/poryscript/font_config.json`
- Route109/scripts.inc MapScripts stub replaced with correct frame table pointing to VAR_ENNA_INTRO_STATE == 22 check
- #define LOCALID_BEA 25 and #define LOCALID_CAPTAIN_DOCK 26 added directly to Route109/scripts.inc header (raw .inc files do not go through C preprocessor)
- include/constants/paradoxia_localids.h added to include/constants/global.h — makes all custom LOCALIDs available to .s files via the C preprocessor include chain
- Water tile collision bug on Route109 dock fixed in Porymap — dock edge tile was a water metatile (collision 0), replaced with correct land/dock edge metatile
- SSEnnaDeck intro scene confirmed firing correctly on VAR_ENNA_INTRO_STATE == 21
- Route109 dock scene in progress — map script frame table wired, scene trigger under investigation
...
5. VAR_ENNA_INTRO_STATE is set to 21 during new game initialisation (after EventScript_ResetAllMapFlags runs)
...
- Custom gender select screen — COMPLETE (Session 12, rebuilt Session 13, rewired Session 14)
...
- All Session 13 linker errors resolved — COMPLETE (Session 14)
- New game spawn wired to SSEnnaDeck at 4,10 — COMPLETE (Session 14)
- VAR_ENNA_INTRO_STATE ordering fix (WarpToTruck moved after flag reset) — COMPLETE (Session 14)
- SSEnnaDeck intro scene firing correctly — COMPLETE (Session 14)
- Water tile collision bug on Route109 dock fixed — COMPLETE (Session 14)
- Route109 dock scene trigger working — COMPLETE (Session 15)
- Lab scene trigger working — COMPLETE (Session 15)
- VAR_ENNA_INTRO_STATE clobber bug fixed — COMPLETE (Session 15)
- Stale .inc duplicate files purged — COMPLETE (Session 15)
- All vanilla Hoenn maps stripped from map_groups.json — COMPLETE (Session 16)
